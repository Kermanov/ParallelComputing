...
void status_check(cl_int status)
{
	if (status != 0)
	{
		std::cout << "ERROR " << status << " on line " << __LINE__ << std::endl;
		std::cin.get();
	}
}


constexpr auto MS_IN_SEC = 1000;
constexpr auto MAX_SOURCE_SIZE = (0x100000);
constexpr auto SOURCE_FILE = "MatrixMultiplication.cl";

const size_t side_size = 2500;
...
int main()
{
	...
	status = clEnqueueWriteBuffer(command_queue, memobj_matrix1, CL_TRUE,
	...
	status = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, workgroup_size, NULL, 0, NULL, NULL);
	status_check(status);
	...
	if (print_matrices)
	{
		...
		printMatrix(gpu_result_matrix);

		std::cout << "CPU:" << std::endl;
		printMatrix(cpu_result_matrix);
	}
